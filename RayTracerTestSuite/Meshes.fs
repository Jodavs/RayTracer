...
module Meshes =
  ...
  let render : Target list =
    List.map (Util.mkTarget "meshes")
      [
       (renderIcosahedron, "icosahedron"); // lucas
       (renderUrn, "urn"); // lucas
       (renderBunny, "bunny"); // lucas
       (renderInsideBunny, "inside bunny"); // lucas
       (renderBunnies 7, "bunnies small"); // lucas 
       (renderBunnies 17, "bunnies big"); // lucas
       (renderDragon, "dragon"); // lucas
       (renderHappy, "happy"); // jonleif
       (renderPorsche, "porsche"); // anton
       (renderHorse, "horse"); // jakob
       (renderHead, "head") // anton
       (renderVase, "vase") // jakob
       (renderArmadillo, "armadillo") // udgået
       ]