...
let private makeSubtreePair axis splitValue (shape: Triangle) =
...
let rec private search (node : KDTreeNode) (ray : Ray) t t' texture (transNormFunc:(Vector->Vector)) =
    match node with
    | Leaf shapes -> 
        let hit = 
            List.fold (fun acc (triangle: Triangle) ->
                // Check if this shape is closer than the closest
                if triangle.HitBoundingBox ray then
                    match triangle.HitFunc transNormFunc [texture] ray with
                    | None -> acc
        ...
        if hit.IsSome && hit.Value.distance < t' then
            ...
            None
    | Node _ -> searchNode node ray t t' texture transNormFunc
   
and private searchNode (node : KDTreeNode) (R(o,d) as ray) t t' texture transNormFunc =
    ...
    if directionAxisValue = 0. then
        ...
        if originAxisValue <= splitValue then
            search left ray t t' texture transNormFunc
        else
            search right ray t t' texture transNormFunc
    else
        ...
        if tHit <= t || tHit <= 0. then
            ...
            if tHit >= t' then
                ...
                let searchFirst = search first ray t tHit texture transNormFunc
                if searchFirst.IsSome then
                    searchFirst
                else
                    search second ray tHit t' texture transNormFunc

let private traverse (tree : KDTree) (R(o,d) as ray) texture transNormFunc =
    ...
    | Some (t, t') -> search root ray t t' texture transNormFunc
...
let public mkKDTree (inputShapes: Triangle list) =
    ...
    let rec inner depth (shapes: Triangle list) axis =
        // Check if there is enough shapes to make a node
        if shapes.Length <= 8 then
            Leaf(shapes)
        else
            let boundingBoxes = List.map (fun (s: Triangle) -> s.GetBoundingBox) shapes 
            ...
            let duplicates = Array.fold (fun acc (left: Triangle option, right: Triangle option) -> if left.IsSome && right.IsSome then acc+1 else acc) 0 shapeDistribution
            ...
            if (float duplicates) / (float shapes.Length) > 0.5 || depth >= maxDepth then
                Leaf(shapes)
            else
    ...
    let boundingBox = getSurroundingBoundingBox (List.map (fun (s: Triangle) -> s.GetBoundingBox) inputShapes)
...
let private mkTreeShape kdTree =
    ...
    let hitFunc transNormFunc (textures: Texture list) ray = 
        traverse kdTree ray textures.[0] transNormFunc // TODO We assume there's at least one

    // Convert into the Tree BaseShape
    new Tree (hitFunc, bb) :> BaseShape

let mkMeshKDTree (triangles: Triangle list) =
    let tree = mkKDTree triangles 

    let countAvgLeaf tree =
        let rec inner tree (numLeaf, count as acc) =
            match tree with
            | Node (_, left, right) -> inner left acc |> inner right
            | Leaf shapes -> (numLeaf+1, count+shapes.Length)
        let numLeafs, numShapes = inner tree (0, 0)
        numShapes/numLeafs
    
    let nodes, _ = tree

    let test = countAvgLeaf nodes
    printfn "Number of Shapes Leaf Node: %A" test
