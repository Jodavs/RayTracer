...
val private makeSubtreePair : axis:int -> splitValue:float -> shape:AbstractShape -> AbstractShape option * AbstractShape option
...
val private search : node:KDTreeNode -> ray:Ray -> t:float -> t':float -> Hit option

val private searchNode : KDTreeNode -> Ray -> float -> float -> Hit option

/// Traverse the tree with the ray and return the hit of a Shape, if it hits any within the tree
val private traverse : tree:KDTree -> ray:Ray -> Hit option
...
val public mkShapeKDTree : shapesInput:AbstractShape list -> BaseShape
